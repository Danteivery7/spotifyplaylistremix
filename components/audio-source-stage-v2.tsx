...
export default function AudioSourceStageV2({ playlist, onReadyChange }: Props) {
  ...
  async function checkSources(baseOverride?: string) {
    ...
    try {
      ...
      setProgress(next.ready ? "Every included song is matched to remixable audio." : `${next.matched}/${next.total} songs matched.`);
    } catch (err) {
      setEngineConnected(false);
      setShowEngineSetup(true);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div style={{ maxWidth: 760 }}>
          ...
          <span style={{ color: "#9ca296", lineHeight: 1.55 }}>
            Spotify has already identified the songs. The remix engine now needs actual audio it can analyze and render. It automatically scans your Windows Music folder and any files/folders you add here.
          </span>
        </div>
        <div className="successBadge" style={{ opacity: engineConnected ? 1 : .7 }}>
          {checking ? "Checking engine…" : engineConnected ? result ? `${result.matched}/${result.total} matched` : "Engine connected" : "Engine not connected"}
        </div>
      </div>
      ...
      {(showEngineSetup || !engineConnected) && (
        <div style={{ marginTop: 16, border: "1px solid #3b4439", borderRadius: 12, padding: 14, background: "#090b09" }}>
          ...
          <div style={{ color: "#9ca296", fontSize: 13, lineHeight: 1.55, marginBottom: 10 }}>
            Run <b>START_PERSONAL_REMIX_HIGH_QUALITY.cmd</b> on your PC, copy the HTTPS <b>trycloudflare.com</b> address it prints, and paste it here. The same engine will handle matching, mixing, video rendering, and downloads.
          </div>
