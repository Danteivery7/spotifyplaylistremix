...
import AudioSourceStage from "@/components/audio-source-stage";
...
export default function PersonalRemixStudioAudio() {
  ...
  const workflow = [
    ...
    { label: "Add audio", done: audioReady, active: Boolean(playlist) && !audioReady },
  ...
  async function importPlaylist(urlOverride?: string) {
    ...
    if (!targetUrl) return setError("Choose a Spotify playlist or paste a playlist link first.");
  ...
  async function startRemix() {
    ...
    if (includedTracks.length < 2) return setError("Keep at least two songs in the mix.");
    if (!audioReady) {
      setError("Add or match the audio sources first. The mix will start once every included song is ready.");
  ...
  return (
    <main className="shell">
      ...
      <section className="hero" id="start-section">
        ...
        <p className="sub">Your playlists live here after Spotify is connected. Add your remixable audio once, then the engine handles song selection, transitions, stems and mastering.</p>
      </section>
      ...
      <section className="card actionCard">
        {!spotifyConnected ? (
          <>
            <div className="actionStep"><div className="actionNumber">1</div><div className="actionCopy"><h2>Connect Spotify once</h2><p>After that, this page can show your playlists directly whenever you come back.</p></div><button className="primary actionButton" onClick={() => connectSpotify()}>Connect Spotify</button></div>
            {showSpotifySetup && (
          ...
          <div style={{ padding: 22 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
              <div><h2 style={{ margin: "0 0 4px" }}>Your Spotify playlists</h2><p style={{ margin: 0, color: "#9ca296" }}>Click one and the full track list loads for review.</p></div>
            ...
            {playlistsLoading && !accountPlaylists.length ? <div style={{ color: "#9ca296" }}>Loading your Spotify playlists…</div> : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 12 }}>
                {accountPlaylists.map((item) => (
                  <button key={item.id} onClick={() => importPlaylist(item.spotifyUrl)} disabled={loading} style={{ textAlign: "left", border: "1px solid #30362e", background: "#0b0e0b", borderRadius: 14, padding: 12, color: "#f4f5ef", cursor: "pointer", minHeight: 94 }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      <div style={{ minWidth: 0 }}><strong style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.name}</strong><span style={{ color: "#9ca296", fontSize: 13 }}>{item.totalTracks ? `${item.totalTracks} songs` : item.ownerName}</span></div>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="actionStep"><div className="actionNumber">↗</div><div className="actionCopy"><h2>Or paste any Spotify playlist link</h2><p>Use this for a playlist that is not already shown above.</p><div className="inputRow"><input className="urlInput" value={playlistUrl} onChange={(event) => setPlaylistUrl(event.target.value)} onKeyDown={(event) => event.key === "Enter" && importPlaylist()} placeholder="https://open.spotify.com/playlist/..." aria-label="Spotify playlist URL" disabled={loading} /><button className="secondary" onClick={() => importPlaylist()} disabled={loading || !playlistUrl.trim()}>{loading ? "Loading…" : "Load Link"}</button></div></div></div>
      ...
      {playlist && (
        <section className="card playlist" id="review-section">
          ...
          <div className="mixSummary"><div><span>In mix</span><strong>{includedTracks.length} songs</strong></div><div><span>Excluded</span><strong>{excludedTrackKeys.length}</strong></div><div><span>Audio</span><strong>{audioReady ? "Ready" : "Needs sources"}</strong></div><div><span>Transitions</span><strong>Phrase-aligned</strong></div></div>
          ...
          {!playlist.truncated && remixPlaylist && includedTracks.length >= 2 && <AudioSourceStage playlist={remixPlaylist} onReadyChange={setAudioReady} />}

          <div className="createBar">
            <div><strong>{playlist.truncated ? "Load the full playlist first" : audioReady ? `${includedTracks.length} songs have audio and are ready` : "Add audio for the included songs"}</strong><span>{audioReady ? "The engine can now analyze the real audio and build the mix." : "Upload files above, or X out songs you do not want to source."}</span></div>
            <div className="createActions">
              ...
              <button className="primary createButton" onClick={startRemix} disabled={playlist.truncated || includedTracks.length < 2 || !audioReady || starting || RUNNING_STATES.includes(job?.state ?? "")}>
                {playlist.truncated ? "Full Playlist Needed" : !audioReady ? "Add Audio First" : starting ? "Starting Mix…" : RUNNING_STATES.includes(job?.state ?? "") ? "Mix Is Running" : "Create My Mix"}
              </button>
            </div>
          </div>
        </section>
      ...
      <p className="legal">Spotify supplies playlist identity and metadata. Songs you X out are never sent to the remix engine. Uploaded audio stays in your configured personal remix engine library.</p>
    </main>
  );
}
